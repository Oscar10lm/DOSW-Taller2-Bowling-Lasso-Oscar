 ...
 *
 * <p>Cubre los escenarios principales del motor de juego:
 * gutter balls, ones, spare, strike, décimo frame y juego perfecto.</p>
 */
...
class BowlingGameTest {
    ...
    /** Lanza un spare (x pines + complemento). */
    ...
    void testOneSpare() {
        rollSpare(5);          // spare en frame 1 (5 + 5)
        game.roll(3);          // bono del spare
        rollMany(17, 0);       // resto de la partida
    ...
    void testOneStrike() {
        game.roll(10);         // strike en frame 1
        game.roll(3);
        game.roll(4);
        rollMany(16, 0);
    ...
    void testTenthFrameSpare() {
        ...
        rollSpare(7);          // décimo frame: spare (7 + 3)
    ...
    void testTenthFrameStrike() {
        rollMany(18, 0);       // frames 1–9: todos cero
        game.roll(10);         // strike en décimo frame
        game.roll(7);
        game.roll(3);
    ...
    @Test
    @DisplayName("isGameOver() retorna false al inicio y true al completar la partida")
    void testIsGameOver() {
        assertFalse(game.isGameOver(), "El juego no debería haber terminado");
        rollMany(20, 0);
        assertTrue(game.isGameOver(), "El juego debería haber terminado");
    }
    ...
    @Test
    @DisplayName("getFrames() retorna exactamente 10 frames")
    void testGetFramesReturnsTenFrames() {
        assertEquals(10, game.getFrames().length);
    }

    @Test
    @DisplayName("Juego con spare en el último frame y strike de bonus → suma correcta")
    ...
    @DisplayName("Tests parametrizados: todos los tiros con N pines")
